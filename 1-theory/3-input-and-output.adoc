...
specification. This book will cover a lot of this extra functionality, but just
like the [levenshtein()] function in PHP, there will be HTTP headers that you
had no idea existed popping up that will make you think, "How the shit did I not
notice that?".
...
JSON is fine. If you have a lot of Ruby folks around, then you will probably
want to output YAML too, which is as easy to generate as JSON in most cases.
