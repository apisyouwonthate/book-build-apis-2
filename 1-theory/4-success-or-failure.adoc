...
GraphQL has an error object format defined, so no choice needs to go into selecting one. It has a message and a location, the location being useful for GraphIQL and other visual query tools to help show which line the error was on.
...
If a request is invalid the message fails. In GraphQL they

Due to the batch-based
nature of GraphQL (trying to do many things in a single request) there are
situations where data and errors can come back at the same time.
...
These validation tools can often be convinced to speak in one of the two common
standard error formats, or you can create your own snowflake error format.
